...
function saveFocusSession(sessionData) {
  ...
  chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    if (message.action === 'inattentive') {
      console.log('Inattentiveness detected via eye tracking...');
      // Handle the inattentiveness event here (e.g., suggest a break, play noise, etc.)
      suggestYoga();  // Example: suggest yoga on inattention
    }
  });
